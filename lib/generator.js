...
function placeBucket(sched, bucketLessons, blocked, relax = false, need = null, teacherDayCapMap = {}) {
  ...
  for (const l of bucketLessons) {
    (teacherGroups[l.teacher] ||= []).push(l);
  }

  // Determine the round template from the teacher with the most groups.
  // Teachers with 1 group participate in ALL rounds (same group each time).
  // Teachers with >1 group rotate through their groups via the template order.
  const entries = Object.entries(teacherGroups);
  entries.sort((a, b) => b[1].length - a[1].length);
  const [templateTeacher, templateGroups] = entries[0];
  shuffleArr(templateGroups);

  const periodsPerLesson = Math.max(...bucketLessons.map(l => l.periods));
  const periods = need == null ? templateGroups.length * periodsPerLesson : need;
  ...
  for (let i = 0; i < periods * 5 && placed < periods; i++) {
    ...
    for (const period of slotOrder) {
      ...
      const slot = slotOf(day, period);
      if (blocked.has(slot)) continue;
      // Determine current round based on template teacher's group order
      const roundIdx = placed % templateGroups.length;
      const templateKey = [...templateGroups[roundIdx].classes].sort().join(",");
      const toPlace = [];
      let ok = true;
      for (const [teacher, groups] of entries) {
        let lesson;
        if (groups.length === 1) {
          lesson = groups[0]; // single-group teacher: always the same
        } else {
          // multi-group teacher: match the template key
          lesson = groups.find(l => [...l.classes].sort().join(",") === templateKey);
        }
        if (!lesson) continue;
        if (!sched.teacherFree(lesson.teacher, slot)) { ok = false; break; }
        if (!lesson.classes.every(c => sched.classFree(lesson.grade, c, slot))) { ok = false; break; }
        const cap = teacherDayCapMap[lesson.teacher];
        if (cap && sched.teacherSlotsOnDay(lesson.teacher, day) >= cap) { ok = false; break; }
        toPlace.push(lesson);
      }
      if (!ok || toPlace.length === 0) continue;
...
function tryGenerate(model) {
  ...
  for (const grade of model.grades) {
    ...
    for (const [bid, ls] of Object.entries(buckets)) {
      // Compute need as max groups per teacher × periods per lesson
      // to handle teachers with multiple merge blocks (e.g. [A,B] + [C,COM])
      const tGroups = {};
      ...
      allBuckets.push({ grade: grade.grade, bid, lessons: ls, need: maxGroups * baseNeed });
    }
  }
