...
function placeBucket(sched, bucketLessons, blocked, relax = false, need = null, teacherDayCapMap = {}) {
  ...
  if (!needAlternate) {
    ...
    return placeBucketTogether(sched, bucketLessons, blocked, relax, need, teacherDayCapMap);
  }
  ...
  const maxPerDay = Math.ceil(totalNeed / 5);
  const dayCount = {};
  let placed = 0;
  const dayOrder = shuffleArr([0, 1, 2, 3, 4]);
  for (let i = 0; i < totalNeed * 5 && placed < totalNeed; i++) {
    const day = dayOrder[i % 5];
...
function placeBucketTogether(sched, bucketLessons, blocked, relax = false, need = null, teacherDayCapMap = {}) {
...
function tryGenerate(model) {
  ...
  for (const { grade: gr, bid, lessons: ls, need } of allBuckets) {
    ...
    const got = placeBucket(sched, ls, blocked, false, need, teacherDayCap);
    if (got < need) {
